...
class TestVoid:
    ...
    def test_add(self):
        assert isinstance(self.f.add(R2Point(0.0, 0.0), self.d), Point)
...
class TestPoint:
    ...
    # Инициализация (выполняется для каждого из тестов класса)
    def setup_method(self):
        d = Circle_perimeter(0, 0, 0)
        self.f = Point(R2Point(0.0, 0.0), d)
...
class TestSegment:
    ...
    def setup_method(self):
        d = Circle_perimeter(0, 0, 0)
        self.f = Segment(R2Point(0.0, 0.0), R2Point(1.0, 0.0), d)
